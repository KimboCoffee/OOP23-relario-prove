...
L'applicazione si basa sull'architettura MVC. Più nello specifico, per la realizzazione della parte Controller
dell'architettura ci si basa sul pattern Observer: quando nella View si registra un evento, il Controller viene immediatamente
notificato e può riflettere eventuali modifiche sul Model, con cui la View non ha interazioni dirette.
\newline Considerando l'andamento del gioco, che procede diversamente durante i combattimenti e al di fuori
di essi, è logico ragionare già di MVC separando le due fasi di gioco. Questo si riflette in particolare sul Model, che
offre al Controller due entry point: l'interfaccia Room per le fasi di gioco standard, e l'interfaccia Fight per le fasi
di combattimento. Questi due entry point avranno poi il compito di dirottare le chiamate del Controller correttamente,
aggiungendo uno strato di separazione fra il Model e il resto dell'architettura. 
\newline L'uso dell'architettura MVC risulta poi affine all'uso di Update Method Pattern, prevedendo per la View e per il
Model un metodo che ne permetta l'aggiornamento. Questo metodo, invocato esternamente rispetto ai costrutti MVC, offre
particolare flessibilità nella realizzazione della parte di View. Infatti, indifferentemente da quale sia la View, ad ogni
"unità temporale" di gioco, sia la View che il Model saranno aggiornati, e solo quando avvengono eventi nella View questi
sono comunicati al Controller che può rifletterli, se necessario sul Model. Organizzando così il programma, non soltanto
sarà possibile cambiare struttura alla GUI, ma sarà anche possibile realizzare altri tipi di View, come per esempio
un'interfaccia da riga di comando. 
