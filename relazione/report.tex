...
L'applicazione si basa sul design architetturale di MVC, nella sua declinazione più generica. Le componenti fondamentali del modello sono
infatti un Model, il quale si occupa di rappresentare tutte le entità del dominio, offrendo un punto d'accesso per la loro modifica, una View
che si occupa dell'interfaccia con l'utente ed è in grado di registrare gli eventi che questo genera, e un Controller che, osservando la View,
gestisce e riflette sul Model eventuali modifiche di stato.
\newline In questo scenario, osservando gli stage di gioco offerti dal modello, si creeranno diversi Controller, in grado di gestire i vari
momenti dell'esecuzione (navigazione dei menù, esplorazione della mappa, combattimento), che saranno accomunati dal fatto di ricevere
messaggi dalla View, deputata all'interfaccia con l'utente.
\newline Anche il Model offre verso l'esterno interfacce diverse per i momenti diversi di gioco, dal momento che non tutti gli elementi
del suo stato sono passibili di modifica in tutti i momenti del gioco (ad esempio, durante l'esplorazione non si può ricevere danno). Per questo
motivo, ogni interfaccia del Model riceverà messaggi da entità diverse del Controller, che a loro volta saranno impegnate a osservare elementi
diversi della View.
